...
__Total number of patrons in presentations = `r tally(Pres, Count)` (average = `r as.integer(tally(Pres, Count)/count(Pres))`)__

__Total presentation time = `r tally(Pres, Length)` minutes (average = `r as.integer(tally(Pres, Length)/count(Pres))` minutes)__
