...
```{r CuserDept}
  CuserDept <- group_by(Cons, UserDept) %>% mutate(total=n()) %>% 
    ...
    filter(total > 1 & UserDept != "NA") %>% arrange(desc(total))
...
```{r PuserDept}
  PuserDept <- group_by(Pres, Department) %>% mutate(total=n()) %>% 
    ...
    filter(total > 1 & Department != "NA") %>% arrange(desc(total))
...
```{r PweekNum}
  ...
  ggplot(data=PweekNum) +
    geom_point(mapping=aes(x=Week, y=total)) +
    geom_line(mapping=aes(x=Week, y=total)) +
    labs(x="Week Number", y="Number of Presentations")
```
